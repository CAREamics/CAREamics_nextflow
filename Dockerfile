...
ARG CAREAMICS_VERSION
RUN pip install --no-cache-dir torch==2.7 torchvision --index-url https://download.pytorch.org/whl/cpu
RUN pip install --no-cache-dir careamics==${CAREAMICS_VERSION}

# gpu build, pytorch base image already includes pytorch
FROM pytorch/pytorch:2.7.1-cuda11.8-cudnn9-runtime AS gpu
ARG CAREAMICS_VERSION
RUN pip install --no-cache-dir careamics==${CAREAMICS_VERSION}